...
const SubCategories = () => {
  ...
  const handleExportPDF = () => {
    const doc = new jsPDF();
    doc.text("Sub Category List", 14, 22);

    const tableColumn = ["Category", "Parent Category", "Code", "Description", "Status"];
    const tableRows = subcateorydata.map((item) => [
      item.category,
      item.parentcategory,
      item.categorycode,
      item.description,
  ...
  const handleExportExcel = () => {
    const csvContent = [
      ["Category", "Parent Category", "Code", "Description", "Status"],
      ...subcateorydata.map((item) => [
        `"${item.category || ""}"`,
        `"${item.parentcategory || ""}"`,
        `"${item.categorycode || ""}"`,
        `"${item.description || ""}"`,
    ...
    link.setAttribute("download", "SubCategoryList.csv");

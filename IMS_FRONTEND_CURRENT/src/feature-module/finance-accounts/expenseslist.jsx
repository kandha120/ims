...
const ExpensesList = () => {
  ...
  const handleExportPDF = () => {
    ...
    const tableColumn = ["Reference", "Category Name", "Description", "Date", "Amount", "Status"];
  ...
  const handleExportExcel = () => {
    const csvContent = [
      ["Reference", "Category Name", "Description", "Date", "Amount", "Status"],
      ...data.map((item) => [
        `"${item.reference || ""}"`,
        `"${item.categoryName || ""}"`,
        `"${item.description || ""}"`,
    ...
    link.setAttribute("download", "ExpenseList.csv");
  ...
  const columns = [
...

    {
      ...
      body: (text) => (
        <span
          className={`badges status-badge fs-10 p-1 px-2 rounded-1 ${text?.status === "Approved" ? "" : "badge-pending"
            }`}
        >
          {text?.status}
        </span>

...
const IncomeList = () => {
  ...
  const handleExportPDF = () => {
    const doc = new jsPDF();
    doc.text("Income List", 14, 22);

    const tableColumn = ["Date", "Reference", "Store", "Category", "Notes", "Amount"];
    ...
    doc.save("IncomeList.pdf");
  };

  const handleExportExcel = () => {
    const csvContent = [
      ["Date", "Reference", "Store", "Category", "Notes", "Amount"],
      ...dataSource.map((item) => [
        `"${item.Date || ""}"`,
        `"${item.Reference || ""}"`,
        `"${item.Store || ""}"`,
    ...
    link.setAttribute("download", "IncomeList.csv");

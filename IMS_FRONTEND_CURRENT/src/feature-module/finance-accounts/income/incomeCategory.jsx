...
const IncomeCategory = () => {
  ...
  const handleExportPDF = () => {
    const doc = new jsPDF();
    doc.text("Income Category List", 14, 22);

    const tableColumn = ["Code", "Category", "Added Date"];
    ...
    doc.save("IncomeCategoryList.pdf");
  };

  const handleExportExcel = () => {
    const csvContent = [
      ["Code", "Category", "Added Date"],
      ...dataSource.map((item) => [
        `"${item.Code || ""}"`,
        `"${item.Category || ""}"`,
        `"${item.Added_Date || ""}"`,
    ...
    link.setAttribute("download", "IncomeCategoryList.csv");
